    ...
    if (cart.length === 0) {
        return (
            <div className="min-h-screen bg-[#050505] flex flex-col items-center justify-center text-white p-10 text-center">
                ...
                <p className="text-white/40 text-base md:text-lg mb-10 max-w-sm md:max-w-md px-6">Your next physical evolution starts with your gear. Explore the store to find your perfect fit.</p>
                <Link to="/shop" className="px-12 py-5 bg-white text-black rounded-full font-bold uppercase tracking-widest hover:bg-[#a14550] hover:text-white transition-all shadow-2xl">Start Exploring</Link>
            </div>
        );
    }
                                    ...
                                    <div className="flex-grow flex flex-col justify-between py-2">
                                        <div className="flex justify-between items-start">
                                            <div className="space-y-2">
                                                <Link to={`/product/${item.slug}`}>
                                                    <h3 className="text-2xl font-bold group-hover:text-[#a14550] transition-colors cursor-pointer">{item.name}</h3>
                                                </Link>
                                        ...
                                        <div className="flex items-center justify-between mt-8">
                                            ...
                                            <button
                                                ...
                                                className="text-white/20 hover:text-[#a14550] transition-colors flex items-center gap-2 font-bold uppercase tracking-widest text-[10px]"
                                            >
                    ...
                    <div className="lg:w-1/3">
                        <div className="sticky top-32 space-y-8">
                            <div className="bg-white/[0.03] border border-white/10 rounded-[40px] p-10 space-y-10 shadow-2xl backdrop-blur-xl">
                                ...
                                <Link
                                    to="/checkout"
                                    className="w-full h-20 bg-[#59000a] text-white rounded-full flex items-center justify-center gap-4 font-black uppercase tracking-[0.2em] hover:bg-[#7a000e] transition-all transform active:scale-[0.98] shadow-2xl"
                                >
                                    Proceed to Checkout
                                        ...
                                        {suggestedProducts.map((p) => (
                                            <Link to={`/product/${p.slug}`} key={p.id} className="flex gap-6 p-4 rounded-[24px] bg-white/[0.02] border border-white/5 hover:bg-white/[0.05] transition-all group">
                                                ...
                                                <div className="flex flex-col justify-center">
                                                    <span className="font-bold text-sm group-hover:text-[#a14550] transition-colors">{p.name}</span>
