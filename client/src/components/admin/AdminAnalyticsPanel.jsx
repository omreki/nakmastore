...
import {
    TrendingUp, Users, MousePointer2,
...
const AdminAnalyticsPanel = () => {
    ...
    const [stats, setStats] = useState({
        totalVisits: 0,
        totalVisitsTrend: null,
        uniqueVisitors: 0,
        uniqueVisitorsTrend: null,
        uniqueSessions: 0,
        conversionRate: '0.0',
        conversionRateTrend: null,
        avgSessionDuration: '0s',
        avgSessionDurationTrend: null,
        abandonedCarts: 0,
        checkoutStarts: 0,
        purchaseCompletes: 0,
        cartAdds: 0,
    ...
    useEffect(() => {
        ...
        const channel = supabase
            .channel('dashboard_analytics_realtime')
            .on('postgres_changes', {
                event: 'INSERT',
                schema: 'public',
    ...
    return (
        <section id="analytics" className="flex flex-col gap-6 md:gap-8 pt-4 border-t border-white/10">
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
                <div>
                    <h2 className="text-white text-2xl md:text-3xl font-black tracking-tight">Store Analytics</h2>
                    <p className="text-gray-400 text-sm font-medium mt-1">
                        Customer flow, engagement, and conversion metrics.
                    </p>
                </div>

                <div className="flex flex-wrap gap-3 p-1.5 bg-black/40 rounded-2xl border border-white/5 shadow-inner">
                    ...
                    {filterType !== 'lifetime' && (
                        <div className="border-t md:border-t-0 md:border-l border-white/10 pt-2 md:pt-0 md:pl-3 flex items-center">
                            {filterType === 'date' ? (
                                <input
                                    type="date"
                                    value={selectedDate}
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 md:gap-8">
                    ...
                    <div className="h-[250px] md:h-[350px] w-full">
                        <ResponsiveContainer width="100%" height="100%">
                            <AreaChart data={visitData}>
                                <defs>
                                    <linearGradient id="dashboardColorVisits" x1="0" y1="0" x2="0" y2="1">
                                        <stop offset="5%" stopColor="#ff007f" stopOpacity={0.3} />
                                        <stop offset="95%" stopColor="#ff007f" stopOpacity={0} />
                                    </linearGradient>
                                    <linearGradient id="dashboardColorInteractions" x1="0" y1="0" x2="0" y2="1">
                                        <stop offset="5%" stopColor="#3b82f6" stopOpacity={0.3} />
                                        <stop offset="95%" stopColor="#3b82f6" stopOpacity={0} />
                                    </linearGradient>
                                </defs>
                                <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#ffffff05" />
                                <XAxis dataKey="date" axisLine={false} tickLine={false} tick={{ fill: '#4b5563', fontSize: 10, fontWeight: 800 }} dy={10} />
                                ...
                                />
                                <Area type="monotone" dataKey="visits" name="Views" stroke="#ff007f" strokeWidth={3} fillOpacity={1} fill="url(#dashboardColorVisits)" />
                                <Area type="monotone" dataKey="interactions" name="Actions" stroke="#3b82f6" strokeWidth={3} fillOpacity={1} fill="url(#dashboardColorInteractions)" />
                            </AreaChart>
                        </ResponsiveContainer>
                    </div>
                </div>
                ...
                <div className="glossy-panel rounded-[2rem] p-6 md:p-8 border border-white/5 shadow-2xl bg-black/40 flex flex-col">
                    ...
                    <div className="flex-1 space-y-8">
                        <FunnelStep label="Manifest Loading" value={stats.cartAdds} percentage={100} color="bg-gray-700" />
                        <FunnelStep label="Checkout Trigger" value={stats.checkoutStarts} percentage={stats.cartAdds > 0 ? (stats.checkoutStarts / stats.cartAdds) * 100 : 0} color="bg-orange-500" />
                        <FunnelStep label="Signal Finalized" value={stats.purchaseCompletes} percentage={stats.checkoutStarts > 0 ? (stats.purchaseCompletes / stats.checkoutStarts) * 100 : 0} color="bg-green-500" />
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 md:gap-8">
                <MatrixBox title="Node Usage" data={popularPages} labelKey="name" valueKey="visits" icon={<FileText className="size-4" />} />
                <MatrixBox title="Object Focus" data={topProducts} labelKey="name" valueKey="views" icon={<Package className="size-4" />} onItemClick={() => navigate('/admin/products')} />
                <MatrixBox title="Control Flux" data={topElements} labelKey="name" valueKey="clicks" icon={<MousePointer2 className="size-4" />} />
                <VisualFeed title="Live Stream" data={interactionData} icon={<Activity className="size-4" />} />
            </div>
        </section>
    );
};
...
const FunnelStep = ({ label, value, percentage, color }) => (
    <div className="space-y-3">
        <div className="flex justify-between items-end">
            ...
            <span className="text-sm font-black text-white tracking-tight">{Math.round(value ?? 0).toLocaleString()}</span>
        </div>
        <div className="h-2 bg-white/5 rounded-full overflow-hidden flex">
            <div className={`h-full ${color} transition-all duration-1000 ease-out shadow-[0_0_15px_rgba(255,255,255,0.05)]`} style={{ width: `${percentage}%` }}></div>
        </div>
        ...
        <div className="space-y-4">
            {data.length > 0 ? data.map((item, idx) => (
                ...
                >
                    ...
                    <span className="text-[10px] font-black text-white ml-2">{(item[valueKey] ?? 0).toLocaleString()}</span>
                </button>
            )) : (
